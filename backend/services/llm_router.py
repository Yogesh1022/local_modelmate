...
# NEW: Utility to hash prompt for unique key
def hash_prompt(prompt: str) -> str:
    """Create a hash of the prompt for use as a unique identifier."""
    return hashlib.sha256(prompt.encode()).hexdigest()

async def call_together_ai(system_prompt: str, user_input: str, **kwargs) -> str:
    """
    Reusable helper to call Together AI with a system prompt and user input.
    """
    if not system_prompt or not user_input:
...
if __name__ == "__main__":
    ...
    async def test():
        try:
            reply = await call_together_ai(
                system_prompt="You are a helpful assistant that generates project plans.",
                user_input="Generate a project plan for a web application"
            )
            logger.info(f"Together AI response: {reply}")
            print(f"🤖 Together AI says:\n{reply}")
        except Exception as e:
